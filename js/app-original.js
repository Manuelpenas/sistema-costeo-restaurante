...
function cargarIngredientes() {
    ...
    if (!tbody) {
        console.error('No se encontró tbody de ingredientes');
        return;
    }
    tbody.innerHTML = '';
    if (ingredientes.length === 0) {
        const tr = document.createElement('tr');
        tr.innerHTML = '<td colspan="8" style="text-align:center; color:#999; padding:20px;">No hay ingredientes registrados</td>';
        tbody.appendChild(tr);
    } else {
        ingredientes.forEach(ing => {
            const tr = document.createElement('tr');
            tr.style.cursor = 'pointer';
            tr.onclick = function() { editarIngrediente(ing.id); };
            tr.innerHTML = `
                <td>${ing.categoria || '-'}</td>
                <td>${ing.nombre || '-'}</td>
                <td>${ing.unidad || '-'}</td>
    ...
    // Actualizar también otros selectores
